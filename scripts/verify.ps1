...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-helix-sec-policy-detail.ps1

php tests/test_domain_review.php

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-helix-sec-policy-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-helix-sec-policy-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-helix_sec_policy_vault-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
